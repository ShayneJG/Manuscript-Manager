...
export default function Search({user}: SearchProps) {
  ...
  useEffect(() => {manuscriptToUpdate ? onOpen() : onClose()}, [manuscriptToUpdate]);
  ...
  const handleSubmit = async () => {
    ...
    if (manuscriptID === "") {
      ...
      try {
        const result = await fetch(`api/manuscripts/getManuscript?identification=${manuscriptID}`);
    ...
    return (
      <main>
        ...
        <FormControl isInvalid={manIdError} isRequired>
          ...
          <Input value={manuscriptID} onChange={(e) => {setManuscriptID(e.target.value)}}></Input>
          <FormErrorMessage>
